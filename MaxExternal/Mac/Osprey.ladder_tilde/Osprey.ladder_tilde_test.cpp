...
#include "Osprey.ladder_tilde.cpp"	// need the source of our object so that we can access it
...
SCENARIO("object produces correct output") {
	...
	GIVEN("An instance of xfade~") {
		...
		test_wrapper<xfade> an_instance;
		xfade&              my_object = an_instance;
		...
		AND_WHEN("The shape is set to 'linear'") {
			THEN("we produce a roughly -6db downpoint in the middle of the fade for both 'fast' and 'precision' modes") {
				...
				REQUIRE(result3 == Approx(0.5).epsilon(std::numeric_limits<float>::epsilon()*100000));
				REQUIRE(result4 == Approx(0.5).epsilon(std::numeric_limits<float>::epsilon()*100000));
			}
		}
